import my_profile_pic from "../images/my_profile_pic.jpg"
import my_pic from "../images/my_pic.jpg"

export const Bio = {
  name: "Utkarsh Nautiyal",
  roles: [
    "Backend Developer",
    "Python Developer",
    "AI/ML Developer",
  ],
  description:
    "Third-year B.Tech student in Computer Science, with a flair for Backend magic, AI/ML wizardry, and Python prowess. Renowned for blending smart work with unwavering dedication, I'm ready to push the boundaries of innovation and make a mark in the tech world.",
  github: "https://github.com/utcrxsh",
  resume:
    "https://drive.google.com/file/d/1qwpuhwDCi6fytBvrl4KC2JKxYudXLXB5/view?usp=drive_link",
  linkedin: "https://www.linkedin.com/in/utkarshnautiyal/",
  insta: "https://www.instagram.com/utcrxsh/",
};

export const skills = [
  {
    title: "Frontend",
    skills: [
      {
        name: "React Js",
        image:
          "data:image/svg+xml;base64,",
      },
      {
        name: "HTML",
        image: "https://www.w3.org/html/logo/badge/html5-badge-h-solo.png",
      },
      {
        name: "CSS",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/CSS3_logo_and_wordmark.svg/1452px-CSS3_logo_and_wordmark.svg.png",
      },
      {
        name: "JavaScript",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6a/JavaScript-logo.png/800px-JavaScript-logo.png",
      },
      {
        name: "Bootstrap",
        image:
          "https://getbootstrap.com/docs/5.3/assets/brand/bootstrap-logo-shadow.png",
      },
    ],
  },
  {
    title: "Backend",
    skills: [
      {
        name: "Python",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg",
      },
      {
        name: "NodeJs",
        image: "https://www.vectorlogo.zone/logos/nodejs/nodejs-icon.svg",
      },
      {
        name: "MySQL",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg",
      },
      {
        name: "MongoDB",
        image: "https://www.vectorlogo.zone/logos/mongodb/mongodb-icon.svg",
      },
    ],
  },
  {
    title: "AI/ML",
    skills: [
      {
        name: "Python",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg",
      },
      {
        name: "Data Structures",
        image:
          "data:image/png;base64,",
      },
      {
        name: "Algorithms",
        image:
          "https://cdn-icons-png.freepik.com/512/15237/15237417.png?ga=GA1.1.119099582.1712822957",
      },
    ],
  },
  {
    title: "Others",
    skills: [
      {
        name: "Git",
        image:
          "https://git-scm.com/images/logo@2x.png",
      },
      {
        name: "GitHub",
        image:
          "https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png",
      },
      {
        name: "Canva",
        image:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTQ7WL31BgP94lXiVCt-m6cReSz8AqGnKVy4hCWa-kh_qEEFDB6byi_OfMP&s=10",
      },
      {
        name: "VS Code",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Visual_Studio_Code_1.35_icon.svg/512px-Visual_Studio_Code_1.35_icon.svg.png?20210804221519",
      },
      {
        name: "Figma",
        image:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRdkzYKPooeDscyj8S3IDiG6AnV4R0IDSH1tg&usqp=CAU",
      },
      {
        name: "ChatGPT",
        image:
          "data:image/png;base64,",
      }
    ],
  },
];

export const experiences = [
  {
    id: 0,
    img: "https://i.ibb.co/Xyg2CCw/images.jpg",
    role: "Backend Developer",
    company: "24hr Hackathon",
    date: "25 nov 2023",
    desc: "Made a Social Media Site with various functionalities",
    skills: [
      "NodeJs",
      "MongoDB",
      "Docker",
      "javascript"
    ],
    doc: "",
  },
  // {
  //   id: 0,
  //   img: "https://media.licdn.com/dms/image/D560BAQEZlhT8nLYPzA/company-logo_200_200/0/1691411694105?e=1728518400&v=beta&t=FhsETcypKXdRLanWDw8tZTUUOxZxRmnxNK8A7LgGjmA",
  //   role: "Club Service Director",
  //   company: "Rotaract Club of Coimbatore Royals",
  //   date: "July 2024 - Present",
  //   desc: "Working on Club Service Projects, managing Event Documents, and Other Membership Details.",
  //   skills: [
  //     "Organization",
  //     "Documentation",
  //     "Event Management"
  //   ],
  //   doc: "",
  // },
  // {
  //   id: 0,
  //   img: "https://media.licdn.com/dms/image/D5603AQFazlrtvvJfLQ/profile-displayphoto-shrink_800_800/0/1706105624313?e=1725494400&v=beta&t=czIfk4Jx70p9YaA8MKfGF3u5jTUw4a5rtXWcY0KOLc8",
  //   role: "International Service Director & Social Media Management",
  //   company: "Rotaract Club of Kathir College of Engineering",
  //   date: "January 2024 - June 2024",
  //   desc: "Working on the International Service Projects, managing event documents, managing club linkedin handle, designing event posters using canva.", 
  //   skills: [
  //     "Canva",
  //     "Documentation",
  //     "Event Management",
  //     "Event Organization"
  //   ],
  //   doc: "",
  // },
];

export const education = [
  {
    id: 0,
    img: "https://i.ibb.co/28pVPsS/download.jpg",
    school: "Graphic Era Hill University",
    date: "2022 - 2026",
    grade: "7.93 CGPA",
    desc: "I am currently pursuing B.Tech in CS at Graphic Era Hill University. I have completed 6 semesters and have a CGPA of 7.93. I have taken courses in Reactjs, Web Development, Machine Learning, Deep Learning. I am was also a member of the GDSC club of Graphic Era Hill University, where I learned additional softskills.",
    degree: "Bachelor of Technology - BTech, CS",
  },
  {
    id: 1,
    img: "https://i.ibb.co/GJXVtzL/download.png",
    school: "Doon International School",
    date: "2020 - 2022",
    grade: "78.8%",
    desc: "I completed my class 12 high school education at Doon International School, where I studied PCM with Computer Science.",
    degree: "CBSE(XII), PCM with Computer",
  },
  {
    id: 2,
    img: "https://i.ibb.co/GJXVtzL/download.png",
    school: "Doon International School",
    date: "Apr 2017 - Apr 2020",
    grade: "93.3%",
    desc: "I completed my class 10 education at Doon International School.",
    degree: "CBSE(X)",
  },
];

export const projects = [
  {
    id: 0,
    title: "ClipConnect",
    date: "July 2024 - Present",
    description:
      "Developed a clipboard sharing site which doesn't require signup and is completely free to use without any autodelete.",
    image:
      "https://i.ibb.co/MDS27MR/Screenshot-61.png",
    tags: ["HTML", "CSS", "Javascript", "Bootstrap", "NodeJs","website"],
    category: "Web",
    github: "https://github.com/utcrxsh/ClipboardConnect",
    webapp: "https://utcurse.tech/",
    member: [
      {
        name: "Utkarsh Nautiyal",
        img: my_pic,
        linkedin: "https://www.linkedin.com/in/utkarshnautiyal/",
        github: "https://github.com/utcrxsh/",
      },
       
    ],
  },
  {
    id: 0,
    title: "Banana Quality Predictor",
    date: "April 2024 - Present",
    description:
      "A Banana Quality predictor Model which tells approxiately for how much time a banana is edible, we have manually classified the dataset for this project.",
    image:
      "https://i.ibb.co/Mcrj3FL/Screenshot-62.png",
    tags: [
      "Tensorflow",
      "Neural Networks",
      "Matplotlib",
      "Python",
      "CNN",
      "Finetuning",
      "Deep Learning"
    ],
    category: "Deep Learning",
    github: "https://www.kaggle.com/code/utkarsh321/banana-classification",
    webapp: "",
    member: [
      {
        name: "Utkarsh Nautiyal",
        img: my_pic,
        linkedin: "https://www.linkedin.com/in/utkarshnautiyal/",
        github: "https://github.com/utcrxsh/",
      },
      {
        name: "Ashish Bhatt",
        img: my_pic,
        linkedin: "https://www.linkedin.com/in/",
        github: "https://github.com/ ",
      },
       
    ]
  },
  // {
  //   id: 1,
  //   title: "Varunah2k24",
  //   date: "Feb 2024 - March 2024",
  //   description:
  //     "Developed Varunah2k24, the official website for Kathir College of Engineering's one-day national level technical symposium. The project was built using HTML, CSS, JavaScript, Bootstrap, and Ajax, ensuring optimal performance and responsiveness on both desktop and mobile platforms. Additionally, created event posters and designs using Canva.",
  //   image:
  //     "https://repository-images.githubusercontent.com/771018463/11dbc197-cd99-4ee0-9eaa-86d02d6e543d",
  //   tags: ["HTML", "CSS", "Javascript", "Bootstrap", "Google Forms", "Canva", "UI/UX Designing"],
  //   category: "Web",
  //   github: "https://github.com/sibisiddharth08/varunahkce.git",
  //   webapp: "https://varunahkce.netlify.app/",
  //   member: [
  //     {
  //       name: "Sibi Siddharth S",
  //       img: my_pic,
  //       linkedin: "https://www.linkedin.com/in/sibisiddharths/",
  //       github: "https://github.com/sibisiddharth8/",
  //     },
  //     
  //   ],
  // },
  // {
  //   id: 2,
  //   title: "ChatSoC",
  //   date: "March 2023 - May 2023 ",
  //   description:
  //     "Developed 'ChatSoC,' an advanced chatbot using OpenAI's API in Python, featuring capabilities such as text-to-voice output, voice-to-text input, QR login mechanism using OpenCV, AI image generation with DALL-E, YouTube video downloading, and Wikipedia content referencing.",
  //   image:
  //     "https://repository-images.githubusercontent.com/699181811/7f39d29e-a66c-4121-a0b0-f5a6a4799e80",
  //   tags: [
  //     "Python",
  //     "Natural Language Processing",
  //     "ML",
  //     "API",
  //     "OpenAI"
  //   ],
  //   category: "Machine Learning",
  //   github: "https://github.com/sibisiddharth8/ChatSoC.git",
  //   webapp: "",
  //   member: [
  //     {
  //       name: "Sibi Siddharth S",
  //       img: my_pic,
  //       linkedin: "https://www.linkedin.com/in/sibisiddharths/",
  //       github: "https://github.com/sibisiddharth8/",
  //     },
  //     
  //   ],
  // },
];


